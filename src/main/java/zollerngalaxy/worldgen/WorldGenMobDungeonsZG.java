...
public class WorldGenMobDungeonsZG extends ZGWorldGenMaster {
	
	private static final Logger LOGGER = ZGHelper.getLogger();
	
	private ResourceLocation lootTable = null;
	private IBlockState blockState = null;
	
	public WorldGenMobDungeonsZG(ResourceLocation lootTableIn, IBlockState blockStateIn) {
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		if (lootTable != null & blockState != null) {
			int i = 3;
			...
			if (j2 >= 1 && j2 <= 5) {
						...
						if (worldIn.isAirBlock(blockpos2)) {
							...
							if (j3 == 1) {
								...
								if (tileentity1 instanceof TileEntityChest) {
									((TileEntityChest) tileentity1).setLootTable(this.lootTable, rand.nextLong());
								}
				...
				if (ZollernGalaxyCore.instance().isInTestMode()) {
					ZGHelper.Log("ZG MOB DUNGEON: " + this.getSpawnedAtString(position));
				}
			...
			ZGHelper.LogWarn("Could not spawn ZG MOB SPAWNER due to null parameters.");
			return false;
		}
	}
