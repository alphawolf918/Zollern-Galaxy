...
public class WorldGenKronosDungeons extends ZGWorldGenMaster {
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		...
		if (j2 >= 1 && j2 <= 5) {
				...
				for (int i4 = 3; i4 >= -1; --i4) {
					for (int k4 = l1; k4 <= i2; ++k4) {
						...
						if (k3 != k && i4 != -1 && k4 != l1 && k3 != l && i4 != 4 && k4 != i2) {
							...
							worldIn.setBlockState(blockpos1, ZGBlocks.blockRedObsidian.getDefaultState(), 2);
						}
					}
				}
			}
					...
					if (worldIn.isAirBlock(blockpos2)) {
						...
						if (j3 == 1) {
							...
							if (tileentity1 instanceof TileEntityChest) {
								((TileEntityChest) tileentity1).setLootTable(LootTableList.CHESTS_NETHER_BRIDGE, rand.nextLong());
							}
			...
			if (ZollernGalaxyCore.instance().isInTestMode()) {
				ZGHelper.Log("NETHER DUNGEON: " + this.getSpawnedAtString(position));
			}
