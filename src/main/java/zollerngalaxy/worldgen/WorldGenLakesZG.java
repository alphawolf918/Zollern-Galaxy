...
public class WorldGenLakesZG extends ZGWorldGenMaster {
	...
	public WorldGenLakesZG(Block blockIn, Block stoneBlockIn) {
		this.blockToGenerate = blockIn;
		this.stoneBlock = stoneBlockIn;
	}
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		...
		if (position.getY() <= 4) {
					...
					for (int k = 0; k < 8; ++k) {
						...
						if (flag) {
							...
							if (k < 4 && !material.isSolid()
									&& worldIn.getBlockState(position.add(k1, k, l2)).getBlock() != this.blockToGenerate) {
								return false;
							}
						}
					}
					...
					for (int i4 = 0; i4 < 8; ++i4) {
						if (aboolean[(l1 * 16 + i3) * 8 + i4]) {
							worldIn.setBlockState(position.add(l1, i4, i3),
									i4 >= 4 ? Blocks.AIR.getDefaultState() : this.blockToGenerate.getDefaultState(), 3);
						}
					}
				}
			}
			
			if (this.blockToGenerate.getDefaultState().getMaterial() == Material.LAVA) {
				for (int j2 = 0; j2 < 16; ++j2) {
					for (int k3 = 0; k3 < 16; ++k3) {
						for (int k4 = 0; k4 < 8; ++k4) {
							boolean flag1 = !aboolean[(j2 * 16 + k3) * 8 + k4] && (j2 < 15 && aboolean[((j2 + 1) * 16 + k3) * 8 + k4]
									|| j2 > 0 && aboolean[((j2 - 1) * 16 + k3) * 8 + k4] || k3 < 15 && aboolean[(j2 * 16 + k3 + 1) * 8 + k4]
									|| k3 > 0 && aboolean[(j2 * 16 + (k3 - 1)) * 8 + k4] || k4 < 7 && aboolean[(j2 * 16 + k3) * 8 + k4 + 1]
			...
			if (this.blockToGenerate.getDefaultState().getMaterial() == Material.WATER) {
				for (int k2 = 0; k2 < 16; ++k2) {
					for (int l3 = 0; l3 < 16; ++l3) {
						...
						if (worldIn.canBlockFreezeWater(position.add(k2, 4, l3))) {
							...
							worldIn.setBlockState(position.add(k2, 4, l3), Blocks.PACKED_ICE.getDefaultState(), flag);
						}
					}
				}
			}
