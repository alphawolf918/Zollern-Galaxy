...
public class StructureComponentVillageWellZG extends StructureComponentVillageZG {
	
	private int averageGroundLevel = -1;
	
	public StructureComponentVillageWellZG() {
	}
	
	public StructureComponentVillageWellZG(StructureComponentVillageStartPieceZG par1ComponentVillageStartPiece, int par2, Random par3Random, int par4, int par5) {
	...
	public void buildComponent(StructureComponent par1StructureComponent, List<StructureComponent> par2List, Random par3Random) {
		StructureVillagePiecesZG.getNextStructureComponentVillagePath((StructureComponentVillageStartPieceZG) par1StructureComponent, par2List, par3Random,
				this.boundingBox.minX - 1, this.boundingBox.maxY - 4, this.boundingBox.minZ + 1, EnumFacing.getHorizontal(1), this.getComponentType());
		StructureVillagePiecesZG.getNextStructureComponentVillagePath((StructureComponentVillageStartPieceZG) par1StructureComponent, par2List, par3Random,
				this.boundingBox.maxX + 1, this.boundingBox.maxY - 4, this.boundingBox.minZ + 1, EnumFacing.getHorizontal(3), this.getComponentType());
		StructureVillagePiecesZG.getNextStructureComponentVillagePath((StructureComponentVillageStartPieceZG) par1StructureComponent, par2List, par3Random,
				this.boundingBox.minX + 1, this.boundingBox.maxY - 4, this.boundingBox.minZ - 1, EnumFacing.getHorizontal(2), this.getComponentType());
		StructureVillagePiecesZG.getNextStructureComponentVillagePath((StructureComponentVillageStartPieceZG) par1StructureComponent, par2List, par3Random,
				this.boundingBox.minX + 1, this.boundingBox.maxY - 4, this.boundingBox.maxZ + 1, EnumFacing.getHorizontal(0), this.getComponentType());
	}
	...
	public boolean addComponentParts(World par1World, Random par2Random, StructureBoundingBox par3StructureBoundingBox) {
		...
		this.fillWithBlocks(par1World, par3StructureBoundingBox, 1, 0, 1, 4, 12, 4, MapGenVillageZG.instance().getHouseBlock(), Blocks.FLOWING_WATER.getDefaultState(),
				false);
		...
		this.fillWithBlocks(par1World, par3StructureBoundingBox, 1, 15, 1, 4, 15, 4, MapGenVillageZG.instance().getHouseBlock(),
				MapGenVillageZG.instance().getHouseBlock(), false);
		
		for (int var4 = 0; var4 <= 5; ++var4) {
			for (int var5 = 0; var5 <= 5; ++var5) {
				if (var5 == 0 || var5 == 5 || var4 == 0 || var4 == 5) {
					this.setBlockState(par1World, Blocks.PLANKS.getDefaultState().withProperty(BlockPlanks.VARIANT, BlockPlanks.EnumType.SPRUCE), var5, 11, var4,
