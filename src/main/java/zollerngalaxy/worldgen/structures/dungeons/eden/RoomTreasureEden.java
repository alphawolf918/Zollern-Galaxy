...
public class RoomTreasureEden extends SizedPieceEden {
	
	public static ResourceLocation DUNGEON_CHEST = new ResourceLocation(ZGInfo.MOD_ID, "chests/dungeonchest_t4a");
	public static final ResourceLocation PLANET_DUNGEON = LootTableList.register(DUNGEON_CHEST);
	
	public RoomTreasureEden() {
	}
	
	public RoomTreasureEden(DungeonConfigurationEden configuration, Random rand, int blockPosX, int blockPosZ, EnumFacing entranceDir) {
		this(configuration, rand, blockPosX, blockPosZ, rand.nextInt(4) + 6, configuration.getRoomHeight(), rand.nextInt(4) + 6, entranceDir);
	}
	
	public RoomTreasureEden(DungeonConfigurationEden configuration, Random rand, int blockPosX, int blockPosZ, int sizeX, int sizeY, int sizeZ, EnumFacing entranceDir) {
				...
				for (int k = 0; k <= this.sizeZ; k++) {
					if (i == 0 || i == this.sizeX || j == 0 || j == this.sizeY || k == 0 || k == this.sizeZ) {
						...
						if (chunkBox.isVecInside(blockpos)) {
							worldIn.setBlockState(blockpos, ZGBlocks.treasureChestT4.getDefaultState().withProperty(ZGBlockTreasureChest.FACING, this.getDirection().getOpposite()), 2);
	...
	public PieceEden getNextPiece(DungeonStartEden startPiece, Random rand) {
		if (startPiece.attachedComponents.size() > 2) {
			...
			if (component instanceof RoomBossEden) {
				...
				((RoomBossEden) component).setChestPos(new BlockPos(blockpos));
			}
		}
		return null;
	}
