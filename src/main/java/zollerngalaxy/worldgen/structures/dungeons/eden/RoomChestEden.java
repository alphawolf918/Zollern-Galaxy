...
public class RoomChestEden extends RoomEmptyEden {
	
	public RoomChestEden() {
	}
	
	public RoomChestEden(DungeonConfigurationEden configuration, Random rand, int blockPosX, int blockPosZ, int sizeX, int sizeY, int sizeZ, EnumFacing entranceDir) {
	...
	public boolean addComponentParts(World worldIn, Random rand, StructureBoundingBox boundingBox) {
		if (super.addComponentParts(worldIn, rand, boundingBox)) {
			...
			if (chest != null) {
				ResourceLocation chesttype = ZGLootTables.CHEST_DUNGEON_TIER4_DEFAULT;
