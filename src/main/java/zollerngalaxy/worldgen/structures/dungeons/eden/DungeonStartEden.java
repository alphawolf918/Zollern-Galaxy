...
public class DungeonStartEden extends EntranceCraterEden {
	...
	public DungeonStartEden(World world, DungeonConfigurationEden configuration, Random rand, int blockPosX, int blockPosZ) {
	...
	public void buildComponent(StructureComponent componentIn, List<StructureComponent> listIn, Random rand) {
		...
		while (!validAttempt && attempts < maxAttempts) {
			...
			if (attachedComponents.size() >= 3 && attachedComponents.get(attachedComponents.size() - 1) instanceof RoomTreasureEden && attachedComponents.get(attachedComponents.size() - 3) instanceof RoomBossEden) {
				validAttempt = true;
			}
			attempts++;
		}
