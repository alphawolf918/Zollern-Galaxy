...
public class EntranceCraterTlaloc extends SizedPieceTlaloc {
	
	private final int range = 16;
	
	public EntranceCraterTlaloc() {
	}
	
	public EntranceCraterTlaloc(World world, DungeonConfigurationTlaloc configuration, Random rand, int blockPosX, int blockPosZ) {
			...
			for (int k = -range; k < range; k++) {
				...
				for (int j = maxLevel; j > 1 && helper <= depth; j--) {
					...
					// if (block1 == this.configuration.getBrickBlock() || j != this.sizeY)
					{
						...
						// this.setBlockState(worldIn, Blocks.AIR.getDefaultState(), i + range, j, k
						// + range, boundingBox);
						helper++;
					}
				}
	...
	public PieceTlaloc getNextPiece(DungeonStartTlaloc startPiece, Random rand) {
		return new RoomEntranceTlaloc(this.configuration, rand, this.boundingBox.minX + this.boundingBox.getXSize() / 2, this.boundingBox.minZ + this.boundingBox.getZSize() / 2);
	}
}