...
public class RoomBossTlaloc extends SizedPieceTlaloc {
	
	private BlockPos chestPos;
	
	public RoomBossTlaloc() {
	}
	
	public RoomBossTlaloc(DungeonConfigurationTlaloc configuration, Random rand, int blockPosX, int blockPosZ, EnumFacing entranceDir) {
		this(configuration, rand, blockPosX, blockPosZ, rand.nextInt(6) + 14, rand.nextInt(2) + 8, rand.nextInt(6) + 14, entranceDir);
	}
	
	public RoomBossTlaloc(DungeonConfigurationTlaloc configuration, Random rand, int blockPosX, int blockPosZ, int sizeX, int sizeY, int sizeZ, EnumFacing entranceDir) {
	...
	public boolean addComponentParts(World worldIn, Random random, StructureBoundingBox chunkBox) {
		...
		if (chunkBox.isVecInside(blockpos)) {
			...
			TileEntityDungeonSpawnerTlaloc spawner = (TileEntityDungeonSpawnerTlaloc) worldIn.getTileEntity(blockpos);
			if (spawner != null) {
