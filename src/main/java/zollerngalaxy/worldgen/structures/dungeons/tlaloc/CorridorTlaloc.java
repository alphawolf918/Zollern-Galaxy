...
public class CorridorTlaloc extends SizedPieceTlaloc {
	
	public CorridorTlaloc() {
	}
	
	public CorridorTlaloc(DungeonConfigurationTlaloc configuration, Random rand, int blockPosX, int blockPosZ, int sizeX, int sizeY, int sizeZ, EnumFacing direction) {
	...
	private <T extends SizedPieceTlaloc> T getRoom(Class<?> clazz, DungeonStartTlaloc startPiece, Random rand) {
		try {
			Constructor<?> c0 = clazz.getConstructor(DungeonConfigurationTlaloc.class, Random.class, Integer.TYPE, Integer.TYPE, EnumFacing.class);
			...
			Constructor<?> c1 = clazz.getConstructor(DungeonConfigurationTlaloc.class, Random.class, Integer.TYPE, Integer.TYPE, Integer.TYPE, Integer.TYPE, Integer.TYPE, EnumFacing.class);
	...
	public PieceTlaloc getNextPiece(DungeonStartTlaloc startPiece, Random rand) {
		...
		if (pieceCount > 10 && startPiece.attachedComponents.get(pieceCount - 2) instanceof RoomBossTlaloc) {
			try {
			...
			if (bossRoom) {
				...
				switch (rand.nextInt(3)) {
					case 0:
						return new RoomSpawnerTlaloc(this.configuration, rand, blockX, blockZ, sizeX, sizeY, sizeZ, this.getDirection().getOpposite());
					case 1:
						return new RoomChestTlaloc(this.configuration, rand, blockX, blockZ, sizeX, sizeY, sizeZ, this.getDirection().getOpposite());
					default:
					case 2:
						return new RoomEmptyTlaloc(this.configuration, rand, blockX, blockZ, sizeX, sizeY, sizeZ, this.getDirection().getOpposite());
				}
			}
		}
		return null;
