...
public class DungeonStartTlaloc extends EntranceCraterTlaloc {
	...
	public DungeonStartTlaloc() {
	}
	
	public DungeonStartTlaloc(World world, DungeonConfigurationTlaloc configuration, Random rand, int blockPosX, int blockPosZ) {
	...
	public void buildComponent(StructureComponent componentIn, List<StructureComponent> listIn, Random rand) {
		...
		while (!validAttempt && attempts < maxAttempts) {
			...
			listIn.clear();
			listIn.add(this);
			PieceTlaloc next = getNextPiece(this, rand);
			while (next != null) {
				listIn.add(next);
			...
			if (attachedComponents.size() >= 3 && attachedComponents.get(attachedComponents.size() - 1) instanceof RoomTreasureTlaloc && attachedComponents.get(attachedComponents.size() - 3) instanceof RoomBossTlaloc) {
				validAttempt = true;
			}
			attempts++;
		}
