...
public class CorridorZG extends SizedPieceZG {
	...
	public PieceZG getNextPiece(DungeonStartZG startPiece, Random rand) {
		...
		if (pieceCount > 10 && startPiece.attachedComponents.get(pieceCount - 2) instanceof RoomBossZG) {
			try {
			...
			if (bossRoom) {
				...
				switch (rand.nextInt(3)) {
					case 0:
						return new RoomSpawnerZG(this.configuration, rand, blockX, blockZ, sizeX, sizeY, sizeZ, this.getDirection().getOpposite());
					case 1:
						return new RoomChestZG(this.configuration, rand, blockX, blockZ, sizeX, sizeY, sizeZ, this.getDirection().getOpposite());
					default:
					case 2:
