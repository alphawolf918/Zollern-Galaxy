...
public class ChunkProviderCentotl extends ChunkProviderBase {
	
	public static final IBlockState STONE = ZGBlocks.centoStone.getDefaultState();
	public static final IBlockState AIR = Blocks.AIR.getDefaultState();
	
	public static final double CHUNK_HEIGHT = 50.0D;
	public static final int SEA_LEVEL = 46;
	
	private static final int CHUNK_SIZE_X = 16;
	private static final int CHUNK_SIZE_Z = 16;
	
	private final BiomeDecoratorCentotl biomeDecoratorCentotl = new BiomeDecoratorCentotl();
	private Random rand;
	...
	private MapGenCavesZG caveGenerator = new MapGenCavesZG(STONE.getBlock(), ZGFluids.blockWhiteLavaFluid);
	private final MapGenRavinesZG ravineGenerator = new MapGenRavinesZG(STONE.getBlock());
	private final MapGenVillageZG villageGenerator = new MapGenVillageZG("Centotl", ZGBlocks.blockCrater);
	...
	public static ChunkProviderCentotl INSTANCE;
	
	public ChunkProviderCentotl(World worldIn, long seed, boolean mapFeaturesEnabled) {
		INSTANCE = this;
		this.world = worldIn;
		...
		this.noiseGen4 = new NoiseGeneratorPerlin(this.rand, 5);
	...
	public void createCraters(int chunkX, int chunkZ, ChunkPrimer primer) {
		for (int cx = chunkX - 4; cx <= chunkX + 4; cx++) {
			for (int cz = chunkZ - 4; cz <= chunkZ + 4; cz++) {
				for (int x = 0; x < ChunkProviderCentotl.CHUNK_SIZE_X; x++) {
					for (int z = 0; z < ChunkProviderCentotl.CHUNK_SIZE_Z; z++) {
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getValue(x * ChunkProviderCentotl.CHUNK_SIZE_X + x,
								cz * ChunkProviderCentotl.CHUNK_SIZE_Z + z) / ChunkProviderCentotl.CRATER_PROB) {
