			...
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++) {
				for (int x = 0; x < ChunkProviderMetztli.CHUNK_SIZE_X; x++) {
					for (int z = 0; z < ChunkProviderMetztli.CHUNK_SIZE_Z; z++) {
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getValue(x * ChunkProviderMetztli.CHUNK_SIZE_X + x,
								cz * ChunkProviderMetztli.CHUNK_SIZE_Z + z) / ChunkProviderMetztli.CRATER_PROB) {
