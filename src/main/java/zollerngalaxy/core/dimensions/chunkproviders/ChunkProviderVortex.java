...
public class ChunkProviderVortex extends ChunkProviderBase {
	...
	private final BiomeDecoratorVortex biomeDecoratorVortex = new BiomeDecoratorVortex();
	private Random rand;
	...
	public void createCraters(int chunkX, int chunkZ, ChunkPrimer primer) {
		for (int cx = chunkX - 2; cx <= chunkX + 2; cx++) {
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++) {
				for (int x = 0; x < ChunkProviderVortex.CHUNK_SIZE_X; x++) {
					for (int z = 0; z < ChunkProviderVortex.CHUNK_SIZE_Z; z++) {
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getValue(x * ChunkProviderVortex.CHUNK_SIZE_X + x,
								cz * ChunkProviderVortex.CHUNK_SIZE_Z + z) / ChunkProviderVortex.CRATER_PROB) {
