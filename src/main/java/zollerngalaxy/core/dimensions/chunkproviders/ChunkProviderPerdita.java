...
public class ChunkProviderPerdita extends ChunkProviderBase {
	...
	private final BiomeDecoratorPerdita biomeDecoratorOasis = new BiomeDecoratorPerdita();
	private Random rand;
				...
				for (int x = 0; x < ChunkProviderPerdita.CHUNK_SIZE_X; x++) {
					for (int z = 0; z < ChunkProviderPerdita.CHUNK_SIZE_Z; z++) {
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getValue(x
								* ChunkProviderPerdita.CHUNK_SIZE_X + x, cz * ChunkProviderPerdita.CHUNK_SIZE_Z + z)
								/ ChunkProviderPerdita.CRATER_PROB) {
