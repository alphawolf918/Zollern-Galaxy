...
public class ChunkProviderAltum extends ChunkProviderBase {
	...
	private final BiomeDecoratorAltum biomeDecoratorAltum = new BiomeDecoratorAltum();
	private Random rand;
	...
	public ChunkProviderAltum(World worldIn, long seed, boolean mapFeaturesEnabled) {
		this.world = worldIn;
	...
	public void createCraters(int chunkX, int chunkZ, ChunkPrimer primer) {
		for (int cx = chunkX - 2; cx <= chunkX + 2; cx++) {
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++) {
				for (int x = 0; x < ChunkProviderAltum.CHUNK_SIZE_X; x++) {
					for (int z = 0; z < ChunkProviderAltum.CHUNK_SIZE_Z; z++) {
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getValue(x * ChunkProviderAltum.CHUNK_SIZE_X + x, cz * ChunkProviderAltum.CHUNK_SIZE_Z + z) / ChunkProviderAltum.CRATER_PROB) {
