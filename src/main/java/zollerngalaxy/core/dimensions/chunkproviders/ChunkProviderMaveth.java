...
public class ChunkProviderMaveth extends ChunkProviderBase {
	
	public static final IBlockState STONE = ZGBlocks.mavethStone.getDefaultState();
	public static final IBlockState AIR = Blocks.AIR.getDefaultState();
	
	public static final double CHUNK_HEIGHT = 62.0D;
	public static final int SEA_LEVEL = 32;
	
	private static final int CHUNK_SIZE_X = 16;
	private static final int CHUNK_SIZE_Z = 16;
	
	private final BiomeDecoratorMaveth biomeDecoratorMaveth = new BiomeDecoratorMaveth();
	private Random rand;
	...
	private MapGenCavesZG caveGenerator = new MapGenCavesZG(STONE.getBlock(), ZGFluids.blockFluidRadite);
	private final MapGenRavinesZG ravineGenerator = new MapGenRavinesZG(STONE.getBlock());
	private final MapGenVillageZG villageGenerator = new MapGenVillageZG("Maveth", ZGBlocks.mavethStoneBricks);
	...
	public static ChunkProviderMaveth INSTANCE;
	
	public ChunkProviderMaveth(World worldIn, long seed, boolean mapFeaturesEnabled) {
		INSTANCE = this;
		this.world = worldIn;
	...
	public void createCraters(int chunkX, int chunkZ, ChunkPrimer primer) {
		for (int cx = chunkX - 4; cx <= chunkX + 4; cx++) {
			for (int cz = chunkZ - 4; cz <= chunkZ + 4; cz++) {
				for (int x = 0; x < ChunkProviderMaveth.CHUNK_SIZE_X; x++) {
					for (int z = 0; z < ChunkProviderMaveth.CHUNK_SIZE_Z; z++) {
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getValue(x * ChunkProviderMaveth.CHUNK_SIZE_X + x, cz * ChunkProviderMaveth.CHUNK_SIZE_Z + z) / ChunkProviderMaveth.CRATER_PROB) {
