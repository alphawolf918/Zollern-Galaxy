...
public class ChunkProviderEden extends ChunkProviderBase {
	...
	private MapGenCavesZG caveGenerator = new MapGenCavesZG(ZGBlocks.edenStone);
			...
			for (int l = 0; l < 5; ++l) {
				...
				for (int j1 = -i1; j1 <= i1; ++j1) {
					for (int k1 = -i1; k1 <= i1; ++k1) {
						...
						// // TODO: Test removing?
						// if (biomegenbase1.getBaseHeight() >
						// biomegenbase.getBaseHeight()) {
						// f7 /= 2.0F;
						// }
				...
				for (int x = 0; x < ChunkProviderEden.CHUNK_SIZE_X; x++) {
					for (int z = 0; z < ChunkProviderEden.CHUNK_SIZE_Z; z++) {
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getValue(x
								* ChunkProviderEden.CHUNK_SIZE_X + x, cz * ChunkProviderEden.CHUNK_SIZE_Z + z)
