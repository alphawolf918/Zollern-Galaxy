...
public class ChunkProviderXantheon extends ChunkProviderBase {
	...
	public static final IBlockState STONE = ZGBlocks.xantheonStone.getDefaultState();
	
	public static final double CHUNK_HEIGHT = 42.0D;
	public static final int SEA_LEVEL = 63;
	
	private static final int CHUNK_SIZE_X = 16;
	private static final int CHUNK_SIZE_Z = 16;
	
	private final BiomeDecoratorXantheon biomeDecoratorXantheon = new BiomeDecoratorXantheon();
	private Random rand;
	...
	private MapGenCavesZG caveGenerator = new MapGenCavesZG(ZGBlocks.xantheonStone, ZGFluids.blockWhiteLavaFluid);
	private final MapGenRavinesZG ravineGenerator = new MapGenRavinesZG(ZGBlocks.xantheonStone);
	private final MapGenVillageZG villageGenerator = new MapGenVillageZG("Xantheon", ZGBlocks.xantheonConstructBlock);
	...
	public ChunkProviderXantheon(World worldIn, long seed, boolean mapFeaturesEnabled) {
		this.world = worldIn;
			...
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++) {
				for (int x = 0; x < ChunkProviderXantheon.CHUNK_SIZE_X; x++) {
					for (int z = 0; z < ChunkProviderXantheon.CHUNK_SIZE_Z; z++) {
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getValue(x * ChunkProviderXantheon.CHUNK_SIZE_X + x,
								cz * ChunkProviderXantheon.CHUNK_SIZE_Z + z) / ChunkProviderXantheon.CRATER_PROB) {
