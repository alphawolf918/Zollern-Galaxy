...
public class ChunkProviderToci extends ChunkProviderBase {
	
	public static final IBlockState STONE = ZGBlocks.tociStone.getDefaultState();
	...
	public static final double CHUNK_HEIGHT = 33.0D;
	public static final int SEA_LEVEL = 54;
	
	private static final int CHUNK_SIZE_X = 16;
	private static final int CHUNK_SIZE_Z = 16;
	
	private final BiomeDecoratorToci biomeDecoratorOasis = new BiomeDecoratorToci();
	private Random rand;
	...
	private MapGenCavesZG caveGenerator2 = new MapGenCavesZG(STONE.getBlock(), FUELTONIUM.getBlock());
	private final MapGenRavinesZG ravineGenerator = new MapGenRavinesZG(STONE.getBlock());
	private final MapGenRavinesZG ravineGenerator2 = new MapGenRavinesZG(STONE.getBlock());
	private final MapGenVillageZG villageGenerator = new MapGenVillageZG("Toci", ZGBlocks.tociStoneBricks);
	...
	public static ChunkProviderToci INSTANCE;
	
	public ChunkProviderToci(World worldIn, long seed, boolean mapFeaturesEnabled) {
		this.world = worldIn;
	...
	public void createCraters(int chunkX, int chunkZ, ChunkPrimer primer) {
		for (int cx = chunkX - 2; cx <= chunkX + 2; cx++) {
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++) {
				for (int x = 0; x < ChunkProviderToci.CHUNK_SIZE_X; x++) {
					for (int z = 0; z < ChunkProviderToci.CHUNK_SIZE_Z; z++) {
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getValue(x * ChunkProviderToci.CHUNK_SIZE_X + x,
								cz * ChunkProviderToci.CHUNK_SIZE_Z + z) / ChunkProviderToci.CRATER_PROB) {
