...
public class ChunkProviderAstros extends ChunkProviderBase {
	...
	private final BiomeDecoratorAstros biomeDecoratorAstros = new BiomeDecoratorAstros();
	private Random rand;
	...
	private final MapGenVillageZG villageGenerator = new MapGenVillageZG("Astros", ZGBlocks.astrosRock);
	...
	public void createCraters(int chunkX, int chunkZ, ChunkPrimer primer) {
		for (int cx = chunkX - 2; cx <= chunkX + 2; cx++) {
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++) {
				for (int x = 0; x < ChunkProviderAstros.CHUNK_SIZE_X; x++) {
					for (int z = 0; z < ChunkProviderAstros.CHUNK_SIZE_Z; z++) {
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getValue(x * ChunkProviderAstros.CHUNK_SIZE_X + x,
								cz * ChunkProviderAstros.CHUNK_SIZE_Z + z) / ChunkProviderAstros.CRATER_PROB) {
