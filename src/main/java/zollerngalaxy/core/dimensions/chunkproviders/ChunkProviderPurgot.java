...
public class ChunkProviderPurgot extends ChunkProviderBase {
	...
	public static final IBlockState STONE = ZGBlocks.purgStone.getDefaultState();
	...
	private MapGenCavesZG caveGenerator = new MapGenCavesZG(ZGBlocks.purgStone);
	private final MapGenRavinesZG ravineGenerator = new MapGenRavinesZG(ZGBlocks.purgStone);
	private final MapGenVillageZG villageGenerator = new MapGenVillageZG("Purgot", ZGBlocks.purgRockBricks);
			...
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++) {
				for (int x = 0; x < ChunkProviderPurgot.CHUNK_SIZE_X; x++) {
					for (int z = 0; z < ChunkProviderPurgot.CHUNK_SIZE_Z; z++) {
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getValue(x * ChunkProviderPurgot.CHUNK_SIZE_X + x,
								cz * ChunkProviderPurgot.CHUNK_SIZE_Z + z) / ChunkProviderPurgot.CRATER_PROB) {
