			...
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++) {
				for (int x = 0; x < ChunkProviderOasis.CHUNK_SIZE_X; x++) {
					for (int z = 0; z < ChunkProviderOasis.CHUNK_SIZE_Z; z++) {
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getValue(x * ChunkProviderOasis.CHUNK_SIZE_X + x, cz * ChunkProviderOasis.CHUNK_SIZE_Z + z)
								/ ChunkProviderOasis.CRATER_PROB) {
