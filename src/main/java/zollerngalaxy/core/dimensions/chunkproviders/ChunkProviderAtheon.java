...
public class ChunkProviderAtheon extends ChunkProviderBase {
	...
	private final MapGenVillageZG villageGenerator = new MapGenVillageZG("Atheon", ZGBlocks.atheonConstructBlock);
			...
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++) {
				for (int x = 0; x < ChunkProviderAtheon.CHUNK_SIZE_X; x++) {
					for (int z = 0; z < ChunkProviderAtheon.CHUNK_SIZE_Z; z++) {
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getValue(x * ChunkProviderAtheon.CHUNK_SIZE_X + x,
								cz * ChunkProviderAtheon.CHUNK_SIZE_Z + z) / ChunkProviderAtheon.CRATER_PROB) {
