...
public class RoomSpawnerNG extends DungeonRoom {
	...
	public RoomSpawnerNG(MapGenDungeon dungeon, int posX, int posY, int posZ,
	...
	public void generate(Block[] chunk, byte[] meta, int cx, int cz) {
		...
		if (this.placeBlock(chunk, meta, this.posX + 1, this.posY - 1,
				this.posZ + 1, cx, cz, Blocks.mob_spawner, 0)) {
			this.spawners.add(new ChunkCoordinates(this.posX + 1,
					this.posY - 1, this.posZ + 1));
		}
		if (this.placeBlock(chunk, meta, this.posX + this.sizeX - 1,
				this.posY - 1, this.posZ + this.sizeZ - 1, cx, cz,
				Blocks.mob_spawner, 0)) {
			this.spawners.add(new ChunkCoordinates(this.posX + this.sizeX - 1,
					this.posY - 1, this.posZ + this.sizeZ - 1));
		}
	}
	...
	protected DungeonRoom makeRoom(MapGenDungeon dungeon, int x, int y, int z,
			ForgeDirection dir) {
		return new RoomSpawnerNG(dungeon, x, y, z, dir);
	}
	
	@Override
	protected void handleTileEntities(Random rand) {
		for (ChunkCoordinates spawnerCoords : this.spawners) {
			if (this.worldObj.getBlock(spawnerCoords.posX, spawnerCoords.posY,
					spawnerCoords.posZ) == Blocks.mob_spawner) {
				TileEntityMobSpawner spawner = (TileEntityMobSpawner) this.worldObj
						.getTileEntity(spawnerCoords.posX, spawnerCoords.posY,
								spawnerCoords.posZ);
				if (spawner != null) {
					spawner.func_145881_a().setEntityName(
							RoomSpawnerNG.getMob(rand));
				}
			}
		}
	}
