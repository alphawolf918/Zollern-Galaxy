...
package galaxymod.worldgen.xathius;
...
public class MapGenCavernXathius extends MapGenBaseMeta {
	...
	public void generate(IChunkProvider par1IChunkProvider, World par2World,
			int par3, int par4, Block[] arrayOfIDs, byte[] arrayOfMeta) {
		...
		for (int var11 = par3 - var6; var11 <= par3 + var6; ++var11) {
			for (int var12 = par4 - var6; var12 <= par4 + var6; ++var12) {
				...
				this.recursiveGenerate(par2World, var11, var12, par3, par4,
	...
	@Override
	protected void recursiveGenerate(World par1World, int xChunkCoord,
			int zChunkCoord, int origXChunkCoord, int origZChunkCoord,
			Block[] arrayOfIDs, byte[] arrayOfMeta) {
		if (this.rand.nextInt(100) == 0) {
			...
			this.generateLargeCaveNode(this.rand.nextLong(), origXChunkCoord,
					origZChunkCoord, arrayOfIDs, arrayOfMeta, xPos, yPos, zPos);
		}
	}
	
	protected void generateLargeCaveNode(long par1, int origXChunkCoord,
			int origZChunkCoord, Block[] arrayOfIDs, byte[] arrayOfMeta,
			double xPos, double yPos, double zPos) {
		this.generateCaveNode(par1, origXChunkCoord, origZChunkCoord,
				arrayOfIDs, arrayOfMeta, xPos, yPos, zPos,
				1.0F + this.rand.nextFloat() * 6.0F, 10.0F, 10.0F, -1, -1, 0.2D);
	}
	
	protected void generateCaveNode(long par1, int origXChunkCoord,
			int origZChunkCoord, Block[] arrayOfIDs, byte[] arrayOfMeta,
			double xPos, double yPos, double zPos, float par12, float par13,
			float par14, int par15, int par16, double heightMultiplier) {
		...
		for (final boolean var28 = var25.nextInt(6) == 0; par15 < par16; ++par15) {
			...
			if (var54 || var25.nextInt(4) != 0) {
				...
				{
					int caveMinX = MathHelper.floor_double(xPos - caveWidth)
							- origXChunkCoord * 16 - 1;
					int caveMaxX = MathHelper.floor_double(xPos + caveWidth)
							- origXChunkCoord * 16 + 1;
					int caveMinY = MathHelper.floor_double(yPos - caveHeight) - 1;
					int caveMaxY = MathHelper.floor_double(yPos + caveHeight) + 1;
					int caveMinZ = MathHelper.floor_double(zPos - caveWidth)
							- origZChunkCoord * 16 - 1;
					int caveMaxZ = MathHelper.floor_double(zPos + caveWidth)
							- origZChunkCoord * 16 + 1;
					...
					for (var42 = caveMinX; !isBlockWater && var42 < caveMaxX; ++var42) {
						for (int var43 = caveMinZ; !isBlockWater
								&& var43 < caveMaxZ; ++var43) {
							for (int var44 = caveMaxY + 1; !isBlockWater
									&& var44 >= caveMinY - 1; --var44) {
					...
					for (var42 = caveMinX; var42 < caveMaxX; ++var42) {
						final double var59 = (var42 + origXChunkCoord * 16
								+ 0.5D - xPos)
								/ caveWidth;
						
						for (var45 = caveMinZ; var45 < caveMaxZ; ++var45) {
							final double var46 = (var45 + origZChunkCoord * 16
									+ 0.5D - zPos)
									/ caveWidth;
							
							if (var59 * var59 + var46 * var46 < 1.0D) {
								for (int var50 = caveMaxY - 1; var50 >= caveMinY; --var50) {
									final double var51 = (var50 + 0.5D - yPos)
											/ caveHeight;
									
									if (var59 * var59 + var51 * var51 + var46
											* var46 < 1.0D) {
										if (var51 > -0.7D) {
											int coords = (var42 * 16 + var45)
													* 256 + var50;
											
											if (arrayOfIDs[coords] == BlockList.xathStone) {
								...
								for (int var50 = caveMaxY - 1; var50 >= caveMinY; --var50) {
									...
									if (var59 * var59 + var51 * var51 + var46
										...
										if (var51 > -0.7D) {
											int coords = (var42 * 16 + var45)
													* 256 + var50;
											int coordsAbove = (var42 * 16 + var45)
													* 256 + var50 + 1;
											int coordsBelow = (var42 * 16 + var45)
													* 256 + var50 - 1;
											
											if (Blocks.air == arrayOfIDs[coords]) {
												if (arrayOfIDs[coordsAbove] == BlockList.xathStone
														&& this.rand
																.nextInt(200) == 0) {
													int modifier = 0;
													
													while (Blocks.air == arrayOfIDs[coordsBelow]) {
														arrayOfMeta[coordsBelow] = (byte) (Math
																.abs(modifier) % 3);
														modifier--;
														coordsBelow = (var42 * 16 + var45)
																* 256
																+ var50
																- 1 + modifier;
													}
												} else if (arrayOfIDs[coordsBelow] == BlockList.xathStone
														&& this.rand
																.nextInt(200) == 0) {
													arrayOfIDs[coords] = Blocks.air;
