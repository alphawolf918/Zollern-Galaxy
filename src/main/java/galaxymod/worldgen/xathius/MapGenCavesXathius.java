...
public class MapGenCavesXathius extends MapGenBaseMeta {
	
	public static int BREAK_THROUGH_CHANCE = 1; // 1 in n chance
	...
	protected void generateLargeCaveNode(long par1, int par3, int par4,
			...
			double par10) {
		this.generateCaveNode(par1, par3, par4, blockIdArray, metaArray, par6,
								...
								for (int localZ = l2; localZ < i3; localZ++) {
									...
									if (xfactorSq + zfactorSq < 1.0D) {
										...
										if (yfactor > -0.7D
												&& xfactorSq + yfactorSq
														+ zfactorSq < 1.0D) {
											if (blockIdArray[coords] == BlockList.xathDirt
													&& metaArray[coords] == 0
													|| blockIdArray[coords] == BlockList.xathStone
													&& metaArray[coords] == 0) {
												blockIdArray[coords] = Blocks.air;
											} else if (metaArray[coords] == 0
													&& random
															.nextInt(MapGenCavesXathius.BREAK_THROUGH_CHANCE) == 0) {
												blockIdArray[coords] = Blocks.air;
											}
										}
									}
