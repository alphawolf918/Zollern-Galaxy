...
public class RoomChestsEden extends DungeonRoom {
	...
	public RoomChestsEden(MapGenDungeon dungeon, int posX, int posY, int posZ,
	...
	protected DungeonRoom makeRoom(MapGenDungeon dungeon, int x, int y, int z,
			ForgeDirection dir) {
		return new RoomChestsEden(dungeon, x, y, z, dir);
	}
	
	@Override
	protected void handleTileEntities(Random rand) {
		if (!this.chests.isEmpty()) {
			this.worldObj.setBlock(this.chests.get(0).posX,
					this.chests.get(0).posY, this.chests.get(0).posZ,
					BlockList.edenTreasureChest, 0, 2);
			final TileEntityTreasureChestEden chest = (TileEntityTreasureChestEden) this.worldObj
					.getTileEntity(this.chests.get(0).posX,
							this.chests.get(0).posY, this.chests.get(0).posZ);
