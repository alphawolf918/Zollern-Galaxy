...
public class MapGenCavesZollus extends MapGenBaseMeta {
	
	public static int BREAK_THROUGH_CHANCE = 4; // 1 in n chance
								...
								for (int localZ = l2; localZ < i3; localZ++) {
									...
									if (xfactorSq + zfactorSq < 1.0D) {
										...
										if (yfactor > -0.7D
												&& xfactorSq + yfactorSq
														+ zfactorSq < 1.0D) {
											if (blockIdArray[coords] == BlockList.zolDirt
													&& metaArray[coords] == 0
													|| blockIdArray[coords] == BlockList.zolstone
													&& metaArray[coords] == 0) {
												blockIdArray[coords] = Blocks.air;
											} else if (metaArray[coords] == 0
													&& random
															.nextInt(MapGenCavesZollus.BREAK_THROUGH_CHANCE) == 0) {
												blockIdArray[coords] = Blocks.air;
											}
										}
									}
