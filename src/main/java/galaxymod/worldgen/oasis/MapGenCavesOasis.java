...
public class MapGenCavesOasis extends MapGenBaseMeta {
	
	public static int BREAK_THROUGH_CHANCE = 6; // 1 in n chance
								...
								for (int localZ = l2; localZ < i3; localZ++) {
									...
									if (xfactorSq + zfactorSq < 1.0D) {
										...
										if (yfactor > -0.7D
												&& xfactorSq + yfactorSq
														+ zfactorSq < 1.0D) {
											if (blockIdArray[coords] == BlockList.oasisDirt
													&& metaArray[coords] == 0
													|| blockIdArray[coords] == BlockList.oasisStone
													&& metaArray[coords] == 0) {
												blockIdArray[coords] = Blocks.air;
											} else if (metaArray[coords] == 0
													&& random
															.nextInt(MapGenCavesOasis.BREAK_THROUGH_CHANCE) == 0) {
												blockIdArray[coords] = Blocks.air;
											}
										}
									}
