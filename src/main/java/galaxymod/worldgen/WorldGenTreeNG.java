...
public class WorldGenTreeNG extends WorldGenAbstractTree {
	...
	private boolean isFruitTree = true;
	
	public WorldGenTreeNG(Block wood, Block leaves, int metaWood, int metaLeaves) {
		this(wood, leaves, metaWood, metaLeaves, false, 4, 3, false);
	}
	
	public WorldGenTreeNG(Block wood, Block leaves, int metaWood,
			boolean isFruitTree) {
	...
	public WorldGenTreeNG(Block wood, Block leaves, int metaWood,
			int metaLeaves, boolean doBlockNotify, int minTreeHeight,
			int randomTreeHeight, boolean vinesGrow) {
		super(doBlockNotify);
	...
	public boolean generate(World par1World, Random par2Random, int par3,
			int par4, int par5) {
		final int l = par2Random.nextInt(this.randomTreeHeight)
				+ this.minTreeHeight;
		boolean flag = true;
		...
		if (par4 >= 1 && par4 + l + 1 <= 256) {
			...
			if (!flag) {
				...
				final boolean isSoil = block2.canSustainPlant(par1World, par3,
						par4 - 1, par5, ForgeDirection.UP,
						(IPlantable) BlockList.edenWoodSapling);
				
				if (isSoil && par4 < 256 - l - 1) {
					block2.onPlantGrow(par1World, par3, par4 - 1, par5, par3,
							par4, par5);
					b0 = 3;
					final byte b1 = 0;
					int l1;
						...
						for (i2 = par3 - l1; i2 <= par3 + l1; ++i2) {
							...
							for (int k2 = par5 - l1; k2 <= par5 + l1; ++k2) {
								...
								if (Math.abs(j2) != l1 || Math.abs(l2) != l1
										...
										&& i3 != 0) {
									final Block block1 = par1World.getBlock(i2,
											k1, k2);
									
									if (block1.isAir(par1World, i2, k1, k2)
											|| block1.isLeaves(par1World, i2,
													k1, k2)) {
										final int metadata = this.isFruitTree ? this.metaLeaves > 0 ? par2Random
												.nextInt(4) == 0 ? this.metaLeaves - 1
												: this.metaLeaves
												: this.metaLeaves
												: this.metaLeaves;
					...
					for (k1 = 0; k1 < l; ++k1) {
						...
						if (block.isAir(par1World, par3, par4 + k1, par5)
								|| block.isLeaves(par1World, par3, par4 + k1,
										par5)) {
							this.setBlockAndNotifyAdequately(par1World, par3,
									par4 + k1, par5, this.wood, this.metaWood);
								...
								for (j2 = par5 - l1; j2 <= par5 + l1; ++j2) {
									if (par1World.getBlock(i2, k1, j2)
										...
										if (par2Random.nextInt(4) == 0
												&& par1World.getBlock(i2 - 1,
														k1, j2).isAir(
														par1World, i2 - 1, k1,
														j2)) {
											this.growVines(par1World, i2 - 1,
													k1, j2, 8);
										}
										...
										if (par2Random.nextInt(4) == 0
												&& par1World.getBlock(i2, k1,
														j2 - 1).isAir(
														par1World, i2, k1,
														j2 - 1)) {
											this.growVines(par1World, i2, k1,
													j2 - 1, 1);
										}
								...
								for (i3 = 0; i3 < 4; ++i3) {
									if (par2Random.nextInt(4 - k1) == 0) {
										...
										this.setBlockAndNotifyAdequately(
												...
												par4 + l - 5 + k1,
												par5
														+ Direction.offsetZ[Direction.rotateOpposite[i3]],
												BlockList.edenSpaceFruit,
												l1 << 2 | i3);
									}
								}
							}
