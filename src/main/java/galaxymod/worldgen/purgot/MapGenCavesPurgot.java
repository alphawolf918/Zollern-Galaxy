...
public class MapGenCavesPurgot extends MapGenBaseMeta {
								...
								for (int localZ = l2; localZ < i3; localZ++) {
									...
									if (xfactorSq + zfactorSq < 1.0D) {
										...
										if (yfactor > -0.7D
												&& xfactorSq + yfactorSq
														+ zfactorSq < 1.0D) {
											if (blockIdArray[coords] == BlockList.purgDirt
													&& metaArray[coords] == 0
													|| blockIdArray[coords] == BlockList.purgStone
													&& metaArray[coords] == 0) {
												blockIdArray[coords] = Blocks.air;
											} else if (metaArray[coords] == 0
													&& random
															.nextInt(MapGenCavesPurgot.BREAK_THROUGH_CHANCE) == 0) {
												blockIdArray[coords] = Blocks.air;
											}
										}
									}
