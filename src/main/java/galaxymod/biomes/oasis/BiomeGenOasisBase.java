...
public class BiomeGenOasisBase extends BiomeSpace {
	...
	public BiomeGenOasisBase(int p_i1971_1_) {
		...
		this.biomeDecor.oasisFlowersPerChunk = 4;
	...
	@Override
	public BiomeGenOasisBase setColor(int var1) {
		return (BiomeGenOasisBase) super.setColor(var1);
	}
	...
	protected BiomeDecoratorOasis getBiomeDecorator() {
		return (BiomeDecoratorOasis) this.theBiomeDecorator;
	}
	
	public void genOasisBiomeTerrain(World world, Random rand, Block[] block,
					...
					if (currentBlock == Blocks.stone) {
						...
						if (currentFillerDepth == -1) {
							if (maxFillerDepth <= 0) {
								topBlock = null;
								topMeta = 0;
								fillerBlock = BlockList.oasisGrass;
								fillerMeta = 0;
							...
							if (y >= seaLevel - 2) {
								...
								fillerBlock = BlockList.oasisGrass;
								fillerMeta = 0;
								block[index] = BlockList.oasisDirt;
							} else {
