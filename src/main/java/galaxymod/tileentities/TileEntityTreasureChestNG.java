...
public abstract class TileEntityTreasureChestNG extends TileEntityAdvanced
		implements IInventory, IKeyable, IPacketReceiver {
	...
	public TileEntityTreasureChestNG(int tier) {
	...
	public void updateEntity() {
		...
		if (!this.worldObj.isRemote
			...
			List<?> list = this.worldObj.getEntitiesWithinAABB(
					EntityPlayer.class, AxisAlignedBB.getBoundingBox(
							this.xCoord - f, this.yCoord - f, this.zCoord - f,
		...
		if (this.numUsingPlayers > 0 && this.lidAngle == 0.0F) {
			...
			d0 = this.zCoord + 0.5D;
			this.worldObj.playSoundEffect(d1, this.yCoord + 0.5D, d0,
		...
		if (this.numUsingPlayers == 0 && this.lidAngle > 0.0F
			...
			if (this.lidAngle < f2 && f1 >= f2) {
				...
				this.worldObj.playSoundEffect(d0, this.yCoord + 0.5D, d2,
	...
	public void closeInventory() {
		if (this.getBlockType() != null
				&& this.getBlockType() == this.getTreasureChestBlock()) {
			--this.numUsingPlayers;
			this.worldObj.addBlockEvent(this.xCoord, this.yCoord, this.zCoord,
					this.getBlockType(), 1, this.numUsingPlayers);
			this.worldObj.notifyBlocksOfNeighborChange(this.xCoord,
	...
	public String getInventoryName() {
		return StatCollector.translateToLocal("container."
				+ this.getTreasureChestName() + ".treasurechest.name");
	}
	...
	@Override
	public boolean onValidKeyActivated(EntityPlayer player, ItemStack key,
			int face) {
		if (this.locked) {
			this.locked = false;
			
			if (this.worldObj.isRemote) {
				player.playSound("mpcore:player.unlock.treasure.chest", 1.0F,
						1.0F);
			} else {
