...
import galaxymod.dimensions.providers.purgot.GenLayerPurgot;
...
public class WorldChunkManagerPurgot extends WorldChunkManagerSpace {
	...
	protected WorldChunkManagerPurgot() {
		this.myBiomeCache = new BiomeCache(this);
		this.myBiomesToSpawnIn = new ArrayList();
		this.myBiomesToSpawnIn.add(BiomeList.biomePurgatory);
		this.myBiomesToSpawnIn.add(BiomeList.biomePurgotLimbo);
	}
	
	public WorldChunkManagerPurgot(long seed) {
		this();
		GenLayer[] agenlayer;
		agenlayer = GenLayerPurgot.makeTheWorld(seed);
		this.unzoomedBiomes = agenlayer[0];
		this.zoomedBiomes = agenlayer[1];
	}
	
	public WorldChunkManagerPurgot(World world) {
		this(world.getSeed());
	}
	
	@Override
	public BiomeGenBase getBiome() {
		return BiomeList.biomePurgatory;
	}
	...
	public BiomeGenBase[] getBiomesForGeneration(
		...
		for (int i = 0; i < length * width; i++) {
			if (arrayOfInts[i] >= 0) {
				...
				par1ArrayOfBiomeGenBase[i] = BiomeList.biomePurgatory;
			}
		}
